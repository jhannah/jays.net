...
# gedcomToHTML.pl 1.5.2 -- Dan Pidcock 26 Feb 2003
$version = "1.5.2"; # version number
# Danio@bigfoot.com - www.pidcock.co.uk/gth

# Copyright (c) Dan Pidcock, 1997-2003.
...
# Children > 8 bug (1.51) - Richard Ball, WebbedGed author (Dick Purnell??)
...
$in1 = 0; # What is at level 1 at the mo.
            ...
            # 3=burial(indiv), 4=notes(i), 
            # 50=marriage(fam),
            # 60=note(source),
            # 61=comment(source).

print "gedcomToHtml $version (c) Dan Pidcock 1997-2001\n";
...
$photo_dir = "Pics"; # Directory where the individual's picture files are
...
foreach $note_id (keys %note) {
    $indiv_note{$note_indiv_link{$note_id}} = $indiv_note{$note_indiv_link{$note_id}}.$note{$note_id};
}    
...
if (open(IN_FILE, "tpl_ind_bot.html")) {
    ...
    $html_ind_bot[1] = "<br><a href=\"people.html\">$str_lpeople</a> ";    
    $html_ind_bot[2] = "| <a href=\"surnames.html\">$str_lsurnames</a><p>\n";
}
close(IN_FILE);
...
foreach $indiv_id (keys %indivs) {
    ...
    # open an output file
    unless (open(OUT_FILE, ">$out_dir/$indiv_id.html")) {
    ...
    {
        print OUT_FILE "<a href=\"$indiv_sour{$indiv_id}.html\">$str_source</a><br><br>\n";
    }
	...
	$printComma = 0;
        ...
        if ($print_sources && $indiv_birt_sour{$indiv_id}) {
            if ($printComma)
                {print OUT_FILE "  ";}
            print OUT_FILE "<a href=\"$indiv_birt_sour{$indiv_id}.html\">$str_source</a>";    
        }
        print OUT_FILE "<br>\n";
    }
	...
	$printComma = 0;
        ...
        if ($print_sources && $indiv_deat_sour{$indiv_id}) {
            if ($printComma)
	      {print OUT_FILE "  ";}
            print OUT_FILE "<a href=\"$indiv_deat_sour{$indiv_id}.html\">$str_source</a>";    
        }
        print OUT_FILE "<br>\n";
    }
    ...
    if ($family_table) {
        ...
        if (($father ne "") || ($mother ne "")) {
            # Ancestors
            print OUT_FILE "<tr><td align=center>\n<table border=0>\n";
            print OUT_FILE "<tr>\n";
            print OUT_FILE "<td width=150 align=center><a href=\"$pgfather_id.html\">$pgfather</a></td>\n";
            print OUT_FILE "<td width=150 align=center><a href=\"$pgmother_id.html\">$pgmother</a></td>\n";
            print OUT_FILE "<td width=150 align=center><a href=\"$mgfather_id.html\">$mgfather</a></td>\n";
            print OUT_FILE "<td width=150 align=center><a href=\"$mgmother_id.html\">$mgmother</a></td>\n";
            print OUT_FILE "</tr>\n<tr>\n";
            if (($pgfather ne "") || ($pgmother ne ""))
                {print OUT_FILE "<td colspan=2><img src=\"$treepic_path/tree300.gif\" width=\"300\" height=\"30\"></td>\n";}
            else
                {print OUT_FILE "<td colspan=2></td>\n";}
            ...
            print OUT_FILE "</tr>\n";
            print OUT_FILE "<tr>\n";
            print OUT_FILE "<td colspan=2 align=center><a href=\"$father_id.html\">$father</a></td>\n";
            print OUT_FILE "<td colspan=2 align=center><a href=\"$mother_id.html\">$mother</a></td>\n";
            print OUT_FILE "</tr>\n";
            print OUT_FILE "<tr>\n";
            if (($father ne "") || ($mother ne "")) 
                {print OUT_FILE "<td colspan=4><img src=\"$treepic_path/tree600.gif\" width=\"600\" height=\"30\"></td>\n";}
            else
                {print OUT_FILE "<td colspan=4></td>\n";}
            print OUT_FILE "<tr>\n";
            print OUT_FILE "<th colspan=4 align=center>$indiv_name{$indiv_id}</th>\n";
            print OUT_FILE "</tr>\n";
        ...
        if ($fams_len != 0) {
            ...
            for ($fams_num=0; $fams_num < $fams_len; $fams_num++) {
                ...
                if ($num_children > 0) {
                    ...
                    print OUT_FILE "<td colspan=$cwidth align=center>";
                    print OUT_FILE $str_m;
                    if ($fams_len > 1) {
                        $i = $fams_num+1;
                        print OUT_FILE "($i) ";
                    }
                    print OUT_FILE "<a href=\"$spouse_id.html\">$spouse</a></td></tr>\n";
                    # the children
                    $start_child = 0;
                    while (($num_children - $start_child) > 8) {
                        # print the 8 child tree with extra leg coming down
                        print OUT_FILE "<tr><td colspan=9><img src=\"$treepic_path/tree_c8l.gif\" width=600 height=30></tr>\n";
                        # print the first 8 children
                        $cell_width = 600/9;
                        print OUT_FILE "<tr>";
                        for ($i = $start_child; $i < $start_child+4; $i++) {
                            print OUT_FILE "<td align=center width=67>";
                            print OUT_FILE "<img src=\"$treepic_path/dot-trans.gif\" width=67 height=1><br>";
                            print OUT_FILE "<a href=\"$child[$i].html\"><font size=-1>$indiv_name{$child[$i]}</font></a></td>\n";
                        }
                        print OUT_FILE "<td align=center width=64><img src=\"$treepic_path/tree_l.gif\" width=64 height=60></td>\n";                        
                    ...
                    print OUT_FILE "<tr><td align=center colspan=9>\n";
                    print OUT_FILE "<table border=0 cellspacing=0 cellpadding=0>";
                    print OUT_FILE "<tr><td colspan=$num_left><img src=\"$treepic_path/tree_c$num_left.gif\" width=600 height=30></td></tr>\n";
                    $cell_width = int(600/$num_left);
                    print OUT_FILE "<tr>";
                    for ($i = $start_child; $i < $num_children; $i++) {
                        print OUT_FILE "<td align=center width=$cell_width>";
                        print OUT_FILE "<img src=\"$treepic_path/dot-trans.gif\" width=$cell_width height=1><br>";
                        print OUT_FILE "<a href=\"$child[$i].html\"><font size=-1>$indiv_name{$child[$i]}</font></a></td>\n";
                    }
                    print OUT_FILE "</tr></table></td></tr>\n";
                    
                    print OUT_FILE "</table>\n</td></tr>\n";
                } # end if has children
                else { #no children so just put marriage for consistency
                    print OUT_FILE "<tr>\n";
                    print OUT_FILE "<td align=center>";
                    print OUT_FILE $str_m;
                    if ($fams_len > 1) {
                        $i = $fams_num+1;
                        print OUT_FILE "($i) ";
                    }
                    print OUT_FILE "<a href=\"$spouse_id.html\">$spouse</a><p></td></tr>\n";   
                }
            } # end for each family
        } # end if has spouses
        print OUT_FILE "</table>\n<p>\n";
    ...
    if ($print_family) {
        ...
        # parents
        if ($father ne "") {
            print OUT_FILE "$str_father <a href=\"$father_id.html\">$father</a><br>\n"; 
        }
        if ($mother ne "") {
            print OUT_FILE "$str_mother <a href=\"$mother_id.html\">$mother</a><p>\n"; 
        }    
        print OUT_FILE "<p>\n";
...
if ($print_sources)
{
    ...
    foreach $sour_id (keys %sours) {
        # open an output file
        unless (open(OUT_FILE, ">$out_dir/$sour_id.html")) {
            die "\nCouldn't open output file $out_dir/$sour_id.html\n";
        }
...
unless (open(OUT_FILE, ">$out_dir/people.html")) {
    die "Couldn't open output file $out_dir/people.html\n";
}
if (open(TPL_FILE, "tpl_people.html")) {
    ...
    @fline = (
        ...
        "<a href=\"#ind_id.html\">#ind_surname, #ind_forname</a> (#ind_birt_date - #ind_deat_date) #photo<br>",
        "#end",
        "\n<p>\n<br><a href=\"surnames.html\">$str_lsurnames</a><p>\n",
        "#footer"
         );
}
    $fline = @fline;
        ...
        if (/#main/) { # do the main loop of all people
		   ...
		   $i++;
            ...
            foreach $indiv_id (sort by_surname keys %indivs) {
                ...
                for ($i = 0; $i < $len; $i++) {
                    if ($ns_this || !$ns[$i]) { # print new sentence lines only when this is
                        $line = $out_fmt[$i];
...
#############################################
# make a surname file
unless (open(OUT_FILE, ">$out_dir/surnames.html")) {
    die "Couldn't open output file $out_dir/surnames.html\n";
}
...
if (open(TPL_FILE, "tpl_surnames.html")) {
    ...
    while (<TPL_FILE>) {
        if (/#main/) { # do the main loop of all surnames
            ...
            $out_fmt[$i-1]=""; # get rid of #end
            $len = @out_fmt;
            $old_surname="-o-o-"; # so that empty surnames show up
	    $cur_letter = 'a';
            foreach $indiv_id (sort by_surname keys %indiv_surname) {
                if (lc($indiv_surname{$indiv_id}) ne $old_surname) {
    ...
    foreach $indiv_id (sort by_surname keys %indiv_surname) {
        if (lc($indiv_surname{$indiv_id}) ne $old_surname) {
            print OUT_FILE "<a href=\"people.html#$indiv_surname{$indiv_id}\">";
            if (lc(substr($indiv_surname{$indiv_id},$[,1)) ne $cur_letter) {
                print OUT_FILE "<br>";
                $cur_letter = lc(substr($indiv_surname{$indiv_id},$[,1));
            }
	    print OUT_FILE "$indiv_surname{$indiv_id}</a> ";
            $old_surname = lc($indiv_surname{$indiv_id});
        }
    }
    
    print OUT_FILE "<p><hr width=50%>\n";
    print OUT_FILE "<br><a href=\"people.html\">$str_lpeople</a><p>\n";
    &print_footer;
}
close(OUT_FILE);
...
sub show_reading_status {
	...
	print "$num_indivs individuals, $num_families families\r";
    }
}
...
sub print_footer {
    ...
    print OUT_FILE "Data compiled, maintained, and hosted by Jay Hannah &lt;jay(at)jays(dot)net&gt;. <a href='http://jays.net/genealogy/'>http://jays.net/genealogy/</a><p>\n";
...
sub print_marriages {
    ...
    for ($fams_num=0; $fams_num < $fams_len; $fams_num++) {
        ...
        if ($spouse ne "") {
            print OUT_FILE " <a href=\"$spouse_id.html\">$spouse</a>";
        }
        ...
        {
            print OUT_FILE "  <a href=\"$fam_marr_sour{$fam_id}.html\">$str_source</a>";    
        }
        print OUT_FILE "\n";
        if ($long) {
            &get_child_data($fams[$fams_num]);
            ...
            for ($i = 0; $i < $num_children; $i++) {
                $child_num = $i+1;
                print OUT_FILE "<dd>$str_child $child_num: <a href=\"$child[$i].html\">$indiv_name{$child[$i]}</a><br>\n";
            }
        }
        print OUT_FILE "</dl><p>\n";
    }
